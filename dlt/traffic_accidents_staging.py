...
@dlt.source
def traffic_accidents_staging_source(api_secret_key: str = dlt.secrets.value):
    # print(f"api_secret_key={api_secret_key}")
    return traffic_accidents_staging_resource(api_secret_key)


@dlt.resource(write_disposition="append")
def traffic_accidents_staging_resource(
...
if __name__ == "__main__":
    ...
    pipeline = dlt.pipeline(
        pipeline_name='traffic_accidents_staging',
        destination='snowflake',
        dataset_name='traffic_accidents_staging_data',
    )

    data = list(traffic_accidents_staging_resource())
